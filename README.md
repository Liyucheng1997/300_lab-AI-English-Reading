...
**🔗 在线使用：<https://liyucheng1997.github.io/300_lab-AI-English-Reading/>**（默认调用本机 `http://localhost:8787` 的 Claude Code 反代，走订阅额度）
...
- **在线版（GitHub Pages）**：默认浏览器直连本机 Claude Code 反代 `http://localhost:8787`，Key 填 `unused`，模型填 `haiku` / `sonnet` / `opus`；也可改成 Anthropic 官方 API。设置只存 localStorage。
